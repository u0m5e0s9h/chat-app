...
firebaseAuth.onAuthStateChanged((user) => {
  if (user) {
    ...
    // Initialize presence and typing managers
    presenceManager.init(user);
    typingManager = new TypingManager(roomId);
    typingManager.init(user);
    
    // Setup presence and typing listeners
...
function setupPresenceListener() {
  presenceManager.listenForOnlineUsers((onlineUsers) => {
...
function setupTypingListener() {
  typingManager.listenForTypingUsers((typingUsers) => {
    ...
    if (typingUsers.length === 0) {
      ...
      typingText.textContent = typingManager.formatTypingText(typingUsers);
    }
  });
}
